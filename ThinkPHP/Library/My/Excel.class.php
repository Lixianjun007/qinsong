...
class Excel {
     ...
     */
    public function Export($is_water = false) {
        // 是否有数据
        ...
        foreach ($this->data as $k => $v) {
            ...
            if (is_array($v) && !empty($v)) {
                foreach ($this->title as $tk => $tv) {
                    ...
                    if ($is_water && $tv['col'] == 'H') {
                        $excel->getActiveSheet()->setCellValueExplicit($tv['col'] . $i, ($excel_charset == 0) ? $v[$tk] : iconv('utf-8', $charset, $v[$tk]), \PHPExcel_Cell_DataType::TYPE_NUMERIC);
                    } else {
